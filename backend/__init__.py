...
def format_summary(datasets, time_range):
    bins = [0]*50
    def time_index(v):
        return int((v - time_range.begin).total_seconds()*len(bins) / (time_range.end - time_range.begin).total_seconds())
    ...
    for dataset in datasets:
        ...
        data[key] = {
            ...
            'measurements': [{'name': name} for name, descr in dataset.metadata.measurements_dict.items()],
    ...
    for dataset in datasets:
        ...
        for idx, measurement in enumerate(request.json['measurements']):
            ...
            with source.open() as goo:
                (x,),(y,) = rasterio.warp.transform('EPSG:4326', source.crs, [lon], [lat])
