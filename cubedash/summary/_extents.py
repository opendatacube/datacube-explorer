...
def _select_dataset_extent_query(dt: DatasetType):
    ...
    if footprint_expression is not None and dt.grid_spec and dt.grid_spec.resolution:
        ...
        footprint_expression = func.ST_SimplifyPreserveTopology(footprint_expression, resolution/4)
...
def datasets_by_region(engine, index, product_name, region_code, time_range, limit):
    ...
    query = (
        select(
            postgres_api._DATASET_SELECT_FIELDS
        ).select_from(
            DATASET_SPATIAL.join(
                DATASET,
                DATASET_SPATIAL.c.id == DATASET.c.id
            )
        ).where(
            DATASET_SPATIAL.c.region_code == bindparam('region_code', region_code)
        ).where(
            DATASET_SPATIAL.c.dataset_type_ref == bindparam('dataset_type_ref', product.id)
        )
    )
    if time_range:
        query = query.where(
