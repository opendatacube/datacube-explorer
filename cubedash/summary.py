...
def calculate_summary(product_name: str, time: Range, log=_LOG) -> TimePeriodOverview:
    ...
    dataset_shapes = [
        shape
        for dataset, (shape, was_valid) in datasets
...
def write_product_summary(
    product: DatasetType, store: SummaryStore, log=_LOG
) -> TimePeriodOverview:
    """
...
def _write_year_summary(
    product: DatasetType, year: int, store: SummaryStore, log
) -> TimePeriodOverview:
    summaries = []
...
def _write_month_summary(
    product: DatasetType, year: int, month: int, store: SummaryStore, log
) -> TimePeriodOverview:
    summary = calculate_summary(product.name, utils.as_time_range(year, month), log=log)
