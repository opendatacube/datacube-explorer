...
var ApplicationRoutes = /** @class */ (function () {
  function ApplicationRoutes(
    regionSearchURLPattern,
    regionViewURLPattern,
    datasetURLPattern,
    geojsonRegionsURL,
    geojsonDatasetsURL,
    geojsonFootprintURL
...
var FootprintLayer = /** @class */ (function (_super) {
  ...
  function FootprintLayer(footprintData, showAlone) {
    ...
    return (
      _super.call(this, footprintData, {
        interactive: false,
        ...
        onEachFeature: function (feature, layer) {
          ...
          layer.on({
            ...
            click: function (e) {
              ...
              if (props.count === 1) {
                window.location.href = routes.getRegionViewURL(
                  props.region_code
                );
              } else {
                window.location.href = routes.getRegionSearchURL(
                  props.region_code
                );
              }
            },
...
var OverviewMap = /** @class */ (function (_super) {
  ...
  function OverviewMap(dataLayers, activeLayer, defaultZoom, defaultCenter) {
    ...
    if (activeLayer) {
      ...
      var _loop_1 = function (dataLayer) {
        ...
        // doesnt run
          ...
          requestData(
            ...
            dataLayer.dataURL,
            function (enabled) {
              return (optBox.disabled = !enabled);
            },
            dataLayer.layer
          );
        }
      ...
      for (
        var _i = 0, dataLayers_1 = dataLayers;
        _i < dataLayers_1.length;
        _i++
      ) {
  ...
  OverviewMap.prototype.changeActive = function (d) {
    for (var _i = 0, _a = this.dataLayers; _i < _a.length; _i++) {
      var otherD = _a[_i];
      if (otherD !== d) this.removeLayer(otherD.layer);
    }
  ...
  if (hasDisplayableData) {
    ...
    if (regionData) {
      layers.push(
        new DataLayer(
          "regions",
          routes.geojsonRegionsURL,
          new RegionsLayer(regionData, routes),
          regionData,
          [footprint]
        )
      );
...
function getViewToggle(name) {
  ...
  var el = document.querySelector(
    'input[name="map_display_view"][value="' + name + '"]'
  );
  console.log('Getting view toggle!', el)
  if (!el) {
