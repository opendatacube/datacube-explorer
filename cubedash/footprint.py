        ...
        for tle in tles:
            ...
            if tle[0] == satellite_name:
                ...
                if not any((x['Satellite name'] == satname and x['Orbit'] == orb.get_orbit_number(
                        datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))) for x in schedule):
                ...
                while deltatime < datetime.strptime(str(st), "%Y/%m/%d %H:%M:%S"):
                    ...
                    eastaz = getEffectiveHeading(sat, oi, sat.sublat.real * (180 / math.pi),
                                                 sat.sublong.real * (180 / math.pi), orad, sat._n) + 90
                    westaz = getEffectiveHeading(sat, oi, sat.sublat.real * (180 / math.pi),
                                                 sat.sublong.real * (180 / math.pi), orad, sat._n) + 270
                    ...
                    geoeastpoint.append(
                        Geodesic.WGS84.Direct(sat.sublat.real * 180 / math.pi, sat.sublong.real * 180 / math.pi, eastaz,
                                              swath))
                    geowestpoint.append(
                        Geodesic.WGS84.Direct(sat.sublat.real * 180 / math.pi, sat.sublong.real * 180 / math.pi, westaz,
                                              swath))
                ...
                SWATH_FILENAME = os.path.join(output_path, satname + "." + str(orb.get_orbit_number(
                    datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))) + ".ALICE.orbit_swath.geojson")
                ORBIT_FILENAME = os.path.join(output_path, satname + "." + str(orb.get_orbit_number(
                    datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))) + ".ALICE.orbit_track.geojson")
                ...
                while (tkdeltatime < datetime.strptime(str(tkst), "%Y/%m/%d %H:%M:%S")):
                    ...
                    tkeastaz = getEffectiveHeading(sat, oi, sat.sublat.real * (180 / math.pi),
                                                   sat.sublong.real * (180 / math.pi), orad, sat._n) + 90
                    tkwestaz = getEffectiveHeading(sat, oi, sat.sublat.real * (180 / math.pi),
                                                   sat.sublong.real * (180 / math.pi), orad, sat._n) + 270
                    # TODO use view angle check to refine step from satellite track see IFOV
                    if tle[0] in ("LANDSAT 8", "LANDSAT 7"):
                        tkswath = 185000 / 2
                    ...
                    tkgeoeastpoint.append(
                        Geodesic.WGS84.Direct(sat.sublat.real * 180 / math.pi, sat.sublong.real * 180 / math.pi,
                                              tkeastaz, tkswath))
                ...
                if len(tkpolypoints) > 0:
                    ...
                    if (((attributes['Node'] == "ascending") and (satname in ("AQUA", "SUOMI_NPP"))) or (
                            (attributes['Node'] == "descending") and (satname not in ("AQUA", "SUOMI_NPP")))):
                        ...
                        addtomap(SWATH_FILENAME, satname + "." + str(
                            orb.get_orbit_number(datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))) + ".swath")
                        addtomap(ORBIT_FILENAME, satname + "." + str(
                            orb.get_orbit_number(datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))) + ".orbit")
                ...
                if minutesaway <= period:
                    ...
                    pol = kml.newpolygon(
                        name=satname + '_' + str(orb.get_orbit_number(datetime.strptime(str(rt), "%Y/%m/%d %H:%M:%S"))),
                    ...
                    for x in sorted(schedule, key=lambda k: k['AOS time']):
                        ...
                        if ((datetime.strptime(str(x['LOS time']), "%Y/%m/%d %H:%M:%S")) < (datetime.utcnow())):
                            # Delete output ogr
                            if os.path.exists(os.path.join(output_path, satname + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_swath.geojson")):
                                shutil.move(os.path.join(output_path, satname + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_swath.geojson"), os.path.join(output_path,
                                                                                              satname + "." + str(x[
                                                                                                                      'Orbit']) + ".ALICE.orbit_swath.geojson.OUTOFDATE"))
                            if os.path.exists(os.path.join(output_path, satname + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_track.geojson")):
                                shutil.move(os.path.join(output_path, satname + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_track.geojson"), os.path.join(output_path,
                                                                                              satname + "." + str(x[
                                                                                                                      'Orbit']) + ".ALICE.orbit_track.geojson.OUTOFDATE"))
                    ...
                    if (len(schedule) > 0 and ((
                            datetime.strptime(str(schedule[len(schedule) - 1]['AOS time']), "%Y/%m/%d %H:%M:%S") < (
                            now + timedelta(minutes=period))))):
                        observer.date = (datetime.strptime(str(schedule[len(schedule) - 1]['LOS time']),
                    ...
                    # print "MODIFIED OBSERVER DATE",observer.date
                        ...
                        for y in sorted(satellites):
                            ...
                            html_output.write(
                                '            <li><a href="' + y + ".schedule.html" + '">' + y + " schedule" + '</a></li>' + '\n')
                        html_output.write("		</ul>" + '\n')
                        ...
                        for x in sorted(schedule):
                            if (((x['Node'] == "ascending") and (x['Satellite name'] in ("AQUA", "SUOMI_NPP"))) or (
                                    (x['Node'] == "descending") and (
                                    x['Satellite name'] not in ("AQUA", "SUOMI_NPP"))) and (
                                    int(x['Minutes to horizon']) <= period)):
                                html_output.write("		<tr>" + '\n')
                                html_output.write("			<td>" + str(x['Satellite name']) + "</td>" + '\n')
                                html_output.write('			<td><a href="' + str(x['SWATH_FILENAME']) + '">' + str(
                                    x['Orbit']) + "</a></td>" + '\n')
                                html_output.write('			<td><a href="' + str(x['ORBIT_FILENAME']) + '">' + str(
                                    x['Node']) + "</a></td>" + '\n')
                                html_output.write("			<td>" + str(x['AOS time']) + "</td>" + '\n')
                                html_output.write("			<td>" + str(x['LOS time']) + "</td>" + '\n')
                                html_output.write(
                                    '			<td><a href="' + satname + "_current_position.geojson" + '">' + str(
                                        x['Minutes to horizon']) + "</a></td>" + '\n')
                                html_output.write("		</tr>" + '\n')
                        html_output.write('		</table>' + '\n')
                        ...
                        for x in sorted(schedule):
                            if (((x['Node'] == "ascending") and (x['Satellite name'] in ("AQUA", "SUOMI_NPP"))) or (
                                    ...
                                    x['Satellite name'] not in ("AQUA", "SUOMI_NPP"))) and (
                                    int(x['Minutes to horizon']) <= period)):
                                ogrswath = os.path.join(output_path, str(x['Satellite name']) + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_swath.geojson")
                                ogrtrack = os.path.join(output_path, str(x['Satellite name']) + "." + str(
                                    x['Orbit']) + ".ALICE.orbit_track.geojson")
                                ogrposition = os.path.join(output_path,
                                                           str(x['Satellite name']) + "_current_position.geojson")
                                ...
                                for feature in layer:
                                    ...
                                    geomlist = str(geom).replace('POLYGON ((', '').replace(')', '').replace('[',
                                                                                                            '').replace(
                                        ']', '').split(',')
                                    ...
                                    html_output.write('		]).addTo(map).bindPopup("<b>Satellite: ' + x[
                                        'Satellite name'] + '</b><br /><b>Orbit: ' + str(
                                        x['Orbit']) + '</b><br /><b>Orbit height: ' + str(
                                        x['Orbit height']) + '</b><br /><b>Minutes to horizon: ' + str(
                                        x['Minutes to horizon']) + '</b><br /><b>AOS time: ' + str(
                                        x['AOS time']) + '</b><br /><b>LOS time: ' + str(
                                        x['LOS time']) + '</b>");' + '\n')
                                ...
                                for feature in layertrack:
                                    ...
                                    html_output.write(
                                        "		],{color:'white'}).addTo(map).bindPopup" + '("<b>Satellite: ' + x[
                                            'Satellite name'] + '</b><br /><b>Orbit: ' + str(
                                            x['Orbit']) + '</b><br /><b>Orbit height: ' + str(
                                            x['Orbit height']) + '</b><br /><b>Minutes to horizon: ' + str(
                                            x['Minutes to horizon']) + '</b><br /><b>AOS time: ' + str(
                        ...
                        html_output.write("		L.marker([" + xyz[1] + "," + xyz[
                            0] + "], {icon: myIcon}).addTo(map).bindPopup" + '("<b>Current Position: ' + x[
                                              'Satellite name'] + '</b>");' + '\n')
