...
def test_stac_loading_all_pages(stac_client: FlaskClient):
    ...
    assert len(all_items) == 393, "Expected all 393 test datasets to be returned " \
                                  "from a search"
    ...
    all_items = list(_iter_items_across_pages(
        stac_client,
        (
            f'/stac/search?'
...
def test_stac_collection_items(stac_client: FlaskClient):
    ...
    assert len(all_items) == 306, ("Expected all 306 test high_tide_comp_20p "
